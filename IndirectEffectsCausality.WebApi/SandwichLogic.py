...
class SandwichLogic():  
   ...
   def qvec2_single(self, x, y, a, m, l, mediator_model, target_model):
        """
        Single observation version supporting separate model types
        """
        b0, ba, bm, bl = x[0:4]
        gam0, gama, gaml = x[4:7]
        E_M1_1, E_M1_0 = x[7:9]
        ...
        # Outcome effects based on target model
        if target_model == "logistic":
            I01I00 = plogis(b0 + ba*0 + bm*1 + bl*l) - plogis(b0 + ba*0 + bm*0 + bl*l)
            I10I00 = plogis(b0 + ba*1 + bm*0 + bl*l) - plogis(b0 + ba*0 + bm*0 + bl*l)
            I11I01 = plogis(b0 + ba*1 + bm*1 + bl*l) - plogis(b0 + ba*0 + bm*1 + bl*l)
        ...
        out = np.array([
            ...
            # Mediator potential outcomes
            (E_M1_pred - E_M1_1) * a,
            (E_M1_pred - E_M1_0) * (1-a),
            (E_M0_pred - E_M0_1) * a,
            (E_M0_pred - E_M0_0) * (1-a),
            
            # Outcome effects
            (I01I00 - E_I01I00_1) * a,
            (I01I00 - E_I01I00_0) * (1-a),
            (I10I00 - E_I10I00_1) * a,
            (I10I00 - E_I10I00_0) * (1-a),
            (I11I01 - E_I11I01_1) * a,
            (I11I01 - E_I11I01_0) * (1-a),
   ...
   def compute_jacobian_vectorized(self, sol_x, Y_data, A_data, M_data, L_values, mediator_model, target_model):
        """
        Vectorized computation of Jacobian matrix - much faster than loop
        """
        # Use the fully vectorized version for maximum speed
        return self.compute_jacobian_fully_vectorized(sol_x, Y_data, A_data, M_data, L_values, mediator_model, target_model)

   def compute_meat_matrix_vectorized(self, sol_x, Y_data, A_data, M_data, L_values, mediator_model, target_model):
        """
        Vectorized computation of MEAT matrix - much faster than loop
        """
        # Use the fully vectorized version for maximum speed
        return self.compute_meat_matrix_fully_vectorized(sol_x, Y_data, A_data, M_data, L_values, mediator_model, target_model)
   ...
   def qvec2_logit(self, x, y, a, m, l):
        """
        Legacy function - kept for compatibility
        """
        return self.qvec2_single(x, y, a, m, l, "logistic", "logistic")

   def qvec2_probit(self, x, y, a, m, l):
        """
        Legacy function - kept for compatibility
        """
        return self.qvec2_single(x, y, a, m, l, "probit", "probit")

   def compute_nnt_effects(self, data, exposure, mediator, outcome, confounders, mediator_model, target_model, selected_effects=None):
        """
        ...
        exposure : str
            Name of the exposure variable (A)
        mediator : str
            Name of the mediator variable (M)
        outcome : str
            Name of the outcome variable (Y)
        confounders : list
            List of confounder variable names (L)
        mediator_model : str
            Type of model for mediator ("logistic" or "probit")
        target_model : str
            Type of model for outcome ("logistic" or "probit")
        B : int
            Number of iterations (kept for compatibility, not used in sandwich method)
            
        Returns:
        --------
        dict
            Dictionary containing NNT estimates and confidence intervals
        """
        try:
            ...
            bread = self.compute_jacobian_vectorized(sol_x, Y_data, A_data, M_data, L_values, mediator_model, target_model)
            ...
            for idx, name in enumerate(nnt_names):
                if name in effects_to_compute:
                    ...
                    ci_results[f"CI_{name}_LOWER"] = None
                    ci_results[f"CI_{name}_UPPER"] = None
            ...
            # Add NNT measures and confidence intervals - only for selected effects
            nnt_measures = ["INNT", "DNNT", "NNT", "INNE", "IEIN", "DNNE", "DEIN", "NNE", "EIN"]
            
            for measure in nnt_measures:
                if measure in effects_to_compute:
                    result[measure] = round(ci_results[measure], 2) if ci_results[measure] is not None and not np.isnan(ci_results[measure]) else None
                    result[f"CI_{measure}_LOWER"] = ci_results[f"CI_{measure}_LOWER"]
                    result[f"CI_{measure}_UPPER"] = ci_results[f"CI_{measure}_UPPER"]
                else:
                    result[measure] = None
